...
const SHEET_CELL_MAPPINGS = {
  ...
  inputData: {
    C4: "siteName",                          // Site Name — Cell C4
    C5: "consentPermitNo",                   // Consent/Permit No — Cell C5
    E5: "wocNumber",                         // WOC Number — Cell E5
    H4: "flowmeterTypeH4",                     // Type of Flowmeter(s) part 1 — Cell H4
    H5: "flowmeterTypeH5",                     // Type of Flowmeter(s) part 2 — Cell H5
    H6: "flowmeterSensorSerial",             // Serial number(s) of Flowmeter(s) - Sensor — Cell H6
    C10: "dryW",                             // Dry weather flow — Cell C10
    C9: "maxD",                              // Maximum daily volume — Cell C9
    C6: "maxFFT",                            // Maximum FFT flow rate — Cell C6
    D8: "qmaxF",                             // Qmax of flowmeter — Cell D8
    C69: "visibleScumMark",                  // Scum mark height above zero — Cell C69
    I26: "verificationPlateMeasurement", // Calibration plate measured height — Cell I26
    I27: "verificationInstrumentDisplay", // Flow instrument average display reading — Cell I27
    D23: "verificationHeadError",            // Head error — Cell D23
    D24: "verificationRepeatabilityError",   // Repeatability error — Cell D24
  },
...
function getSheetCategory(sheetName) {
  ...
  // "Input Data" sheet - match "Input Data", "InputData", or "Input Data Sheet"
  if (name.includes("input data") || name.includes("inputdata") || name === "input") {
...
export async function processExcelFile(file) {
  try {
    ...
    // Post-processing: Combine H4+H5 into flowmeterType, then copy to secondaryDeviceType
    const h4val = extractedData.flowmeterTypeH4 != null ? String(extractedData.flowmeterTypeH4).trim() : null;
    const h5val = extractedData.flowmeterTypeH5 != null ? String(extractedData.flowmeterTypeH5).trim() : null;
    if (h4val || h5val) {
      extractedData.flowmeterType = [h4val, h5val].filter(Boolean).join(" ");
      extractedData.secondaryDeviceType = extractedData.flowmeterType;
    }

    // I26 and I27 are mapped directly to verificationPlateMeasurement and verificationInstrumentDisplay
...
export function mapExcelDataToForm(excelData) {
  ...
  // Handle flowmeterType (combined H4+H5)
  if (
    excelData.hasOwnProperty("flowmeterType") &&
    excelData.flowmeterType !== null &&
    excelData.flowmeterType !== undefined
  ) {
    formData.flowmeterType = safeValue(excelData.flowmeterType);
  }
  ...
  if (
    excelData.hasOwnProperty("verificationHeadError") &&
    excelData.verificationHeadError !== null &&
    excelData.verificationHeadError !== undefined
  ) {
    formData.verificationHeadError = safeValue(excelData.verificationHeadError);
  }

  if (
    excelData.hasOwnProperty("verificationRepeatabilityError") &&
    excelData.verificationRepeatabilityError !== null &&
    excelData.verificationRepeatabilityError !== undefined
  ) {
    formData.verificationRepeatabilityError = safeValue(excelData.verificationRepeatabilityError);
  }
  ...
  // maxFFT from C6 takes priority over fftM3
  if (
  ...
  // siteContact, siteAddress, siteRefPostcode, irishGridRef, niwAssetId, inspectionReportNo
  // are manual/from DB — not populated from Excel
  ...
  // For Form 3, wocNumber comes from E5 directly.
  if (
    excelData.hasOwnProperty("wocNumber") &&
    excelData.wocNumber !== null &&
    excelData.wocNumber !== undefined
  ) {
    formData.wocNumber = safeValue(excelData.wocNumber);
  } else if (
    excelData.hasOwnProperty("consentPermitNo") &&
    excelData.consentPermitNo !== null &&
    excelData.consentPermitNo !== undefined &&
  ...
  // Map visibleScumMark from C69 (scum mark height above zero)
  if (
    excelData.hasOwnProperty("visibleScumMark") &&
    excelData.visibleScumMark !== null &&
    excelData.visibleScumMark !== undefined
  ) {
    formData.visibleScumMark = safeValue(excelData.visibleScumMark);
  }
  ...
  // Do NOT store _excelMetadata in formData — it is not a form field
