...
// Unique marker replaced post-render with the equipment table XML
const EQUIPMENT_TABLE_MARKER = "___SURVEY_EQUIPMENT_TABLE___";

// Generate WordprocessingML table for the equipment list
...
// Inject the equipment table XML into the rendered document
const injectEquipmentTable = (zip) => {
  ...
  if (content.includes(EQUIPMENT_TABLE_MARKER)) {
    // Find the <w:p> that contains the marker and replace it with table + empty paragraph
    const markerRegex = new RegExp(`<w:p[^>]*>(?:(?!</w:p>).)*${EQUIPMENT_TABLE_MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:(?!</w:p>).)*</w:p>`, "s");
    const tableXml = buildEquipmentTableXml();
    // Replace the marker paragraph with: the table followed by an empty paragraph
...
export const generateDocumentFromTemplate = async (
  ...
  try {
    ...
    try {
      doc.setData(formData);
      doc.render();
      // Inject the equipment table into the rendered document
...
export const generateMCLERTSReport = async (formData) => {
  const data = {
    ...
    flowmeterSerial: formData.flowmeterSerial || formData.flowmeterSensorSerial || "",
